...
function Phone({ src, alt, width }: { src: string; alt: string; width: number }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: "0 16px 48px rgba(0,0,0,0.2), 0 3px 10px rgba(0,0,0,0.12)",
      overflow: "hidden",
    }}>
  ...
  return (
    <div className="absolute inset-0 overflow-hidden flex flex-col md:flex-row"
      ...
      <div className="hidden md:flex flex-1 items-center justify-center gap-6 px-6 py-6 min-w-0"
        ...
        {/* هاتف بجانب اللابتوب */}
        <Phone src={slide.mobileImage} alt={slide.title} width={138} />
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex flex-1 items-center justify-center gap-8 p-8 min-w-0"
        ...
        <Phone src={slide.image} alt={slide.title} width={230} />
      </div>
      <div className="hidden md:flex flex-shrink-0 w-72 items-center px-7 py-6"
        style={{
          background: "#fff", borderLeft: "1px solid rgba(0,0,0,0.06)",
  ...
  return (
    ...
    >
      <div className="absolute inset-0 pointer-events-none"
        style={{ backgroundImage: `radial-gradient(circle, rgba(167,206,0,0.18) 1px, transparent 1px)`, backgroundSize: "28px 28px", opacity: 0.45 }} />
...
export default function App() {
  ...
  const next = useCallback(() => { setCurrent(c => (c + 1) % slides.length); setProgress(0); }, []);
  const prev = useCallback(() => { setCurrent(c => (c - 1 + slides.length) % slides.length); setProgress(0); }, []);
  ...
  useEffect(() => {
    ...
    timerRef.current    = setInterval(() => { setCurrent(c => (c + 1) % slides.length); setProgress(0); }, SLIDE_DURATION);
    return clearTimers;
  }, [playing, current]);
  ...
  return (
    <div className="flex flex-col overflow-hidden" style={{ width: "100vw", height: "100dvh", fontFamily: "Cairo, sans-serif", background: "#f5f6f8" }}>
      ...
      <div id="slide-viewport" className="relative flex-1 overflow-hidden">
        {slides.map((sl, i) => (
          <div key={i} className="absolute inset-0" style={{ zIndex: i === current ? 1 : 0, pointerEvents: i === current ? "auto" : "none" }}>
            {sl.kind === "intro"        && <IntroSlide visible={i === current} />}
            {sl.kind === "module-intro" && <ModuleIntroSlide slide={sl} visible={i === current} />}
            {sl.kind === "screen"       && <ScreenSlide slide={sl} visible={i === current} />}
            {sl.kind === "pair"         && <PairSlide slide={sl} visible={i === current} />}
            {sl.kind === "mobile"       && <MobileScreenSlide slide={sl} visible={i === current} />}
            {sl.kind === "outro"        && <OutroSlide visible={i === current} />}
          </div>
        ))}
      </div>
